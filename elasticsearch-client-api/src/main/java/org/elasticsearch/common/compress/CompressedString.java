...
public class CompressedString implements Streamable {
    ...
    public CompressedString(BytesReference data) throws IOException {
        ClientCompressor compressor = ClientCompressorFactory.compressor(data);
        if (compressor != null) {
            // already compressed...
            this.bytes = data.toBytes();
        } else {
            BytesArray bytesArray = data.toBytesArray();
            this.bytes = ClientCompressorFactory.defaultCompressor().compress(bytesArray.array(), bytesArray.arrayOffset(), bytesArray.length());
        }
    }
    ...
    public CompressedString(byte[] data, int offset, int length) throws IOException {
        ...
        if (compressor != null) {
            ...
            // default to LZF
            this.bytes = ClientCompressorFactory.defaultCompressor().compress(data, offset, length);
        }
    }

    public CompressedString(String str) throws IOException {
        UnicodeUtil.UTF8Result result = Unicode.unsafeFromStringAsUtf8(str);
        this.bytes = ClientCompressorFactory.defaultCompressor().compress(result.result, 0, result.length);
    }
