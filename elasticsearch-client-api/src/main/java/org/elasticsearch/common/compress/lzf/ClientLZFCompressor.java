...
/**
 */
public class ClientLZFCompressor implements ClientCompressor {
    ...
    public ClientLZFCompressor() {
        this.decoder = ChunkDecoderFactory.optimalInstance();
        Loggers.getLogger(ClientLZFCompressor.class).debug("using [{}] decoder", this.decoder.getClass().getSimpleName());
    }
    ...
    public void configure(Settings settings) {
        ...
        if (decoderType != null) {
            if ("optimal".equalsIgnoreCase(decoderType)) {
                ...
                Loggers.getLogger(ClientLZFCompressor.class).warn("decoder type not recognized [{}], still using [{}]", decoderType, this.decoder.getClass().getSimpleName());
            }
        }
    }
