 ...
 *
 */
public class ClientTransportStreams {
    ...
    public static void buildRequest(ClientCachedStreamOutput.Entry cachedEntry, final long requestId, final String action, final Streamable message, TransportRequestOptions options) throws IOException {
        byte status = 0;
        status = ClientTransportStreams.statusSetRequest(status);

        if (options.compress()) {
            status = ClientTransportStreams.statusSetCompress(status);
            cachedEntry.bytes().write(HEADER_PLACEHOLDER);
            StreamOutput stream = cachedEntry.handles(BasicCompressorFactory.defaultCompressor());
        ...
        ClientTransportStreams.writeHeader(bytes.array(), bytes.arrayOffset(), bytes.length(), requestId, status);
    }

    public static void buildResponse(ClientCachedStreamOutput.Entry cachedEntry, final long requestId, Streamable message, TransportResponseOptions options) throws IOException {
        byte status = 0;
