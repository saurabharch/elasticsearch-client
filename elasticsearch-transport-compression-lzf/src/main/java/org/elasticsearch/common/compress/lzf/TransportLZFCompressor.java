...
/**
 */
public class TransportLZFCompressor extends BasicLZFCompressor implements NettyCompressor {

    private ChunkDecoder decoder;

    public TransportLZFCompressor() {
        this.decoder = ChunkDecoderFactory.optimalInstance();
        Loggers.getLogger(TransportLZFCompressor.class).debug("using [{}] decoder", this.decoder.getClass().getSimpleName());
    }
    ...
    public void configure(Settings settings) {
        ...
        if (decoderType != null) {
            if ("optimal".equalsIgnoreCase(decoderType)) {
                ...
                Loggers.getLogger(TransportLZFCompressor.class).warn("decoder type not recognized [{}], still using [{}]", decoderType, this.decoder.getClass().getSimpleName());
            }
        }
    }
