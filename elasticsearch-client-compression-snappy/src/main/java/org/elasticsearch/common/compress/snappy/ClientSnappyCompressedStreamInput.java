...
/**
 */
public abstract class ClientSnappyCompressedStreamInput extends CompressedStreamInput<SnappyCompressorContext> {
    ...
    public ClientSnappyCompressedStreamInput(StreamInput in, SnappyCompressorContext context) throws IOException {
        super(in, context);
    ...
    public void readHeader(StreamInput in) throws IOException {
        byte[] header = new byte[ClientSnappyCompressor.HEADER.length];
        in.readBytes(header, 0, header.length);
        if (!Arrays.equals(header, ClientSnappyCompressor.HEADER)) {
