...
public class ContentScriptService {
    ...
    private final static ESLogger logger = ESLoggerFactory.getLogger(ContentScriptService.class.getName());

    private final Settings settings;
    
    private final String defaultLang;

    private final ImmutableMap<String, ContentScriptEngineService> scriptEngines;
    ...
    static ImmutableSet<ContentScriptEngineService> getScriptEngineServices() {
        Builder<ContentScriptEngineService> set = ImmutableSet.<ContentScriptEngineService>builder();
        ServiceLoader<ContentScriptEngineService> loader = ServiceLoader.load(ContentScriptEngineService.class);
        Iterator<ContentScriptEngineService> it = loader.iterator();
        while (it.hasNext()) {
            ContentScriptEngineService se = it.next();
            set.add(se);
        }
        return set.build();
    }

    public ContentScriptService(Settings settings) {
        this(settings, new ClientEnvironment(), getScriptEngineServices());
    }

    public ContentScriptService(Settings settings, ClientEnvironment env, Set<ContentScriptEngineService> scriptEngines) {
        this.settings = settings;

        this.defaultLang = settings.get("default_lang", "mvel");
        this.disableDynamic = settings.getAsBoolean("disable_dynamic", false);

        ImmutableMap.Builder<String, ContentScriptEngineService> builder = ImmutableMap.builder();
        for (ContentScriptEngineService scriptEngine : scriptEngines) {
        ...
        for (File file : dir.listFiles()) {
            if (file.isDirectory()) {
                ...
                if (extIndex != -1) {
                    ...
                    boolean found = false;
                    for (ContentScriptEngineService engineService : scriptEngines.values()) {
    ...
    public CompiledScript compile(String lang, String script) {
        ...
        ContentScriptEngineService service = scriptEngines.get(lang);
        if (service == null) {
